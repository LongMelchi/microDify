...
请参考下面这段prompt的格式，我现在需要生成模型管理 http://localhost:3000/provider 具体的页面内容，生成一段适合microDify前端风格格式的prompt，要求需要符合claude.md的结构，同时具体的前端风格必须和 @/d:/Iamster/microDify-v3/docs/frontend-template.html 一致，限制性要求：1.必须遵守claude.md的规范 2.先在对话中显示 3.有疑问可以先使用头脑风暴对我进行提问
参考prompt如下：
...
生成一段prompt，可以使后端完成前端模型管理对应的功能，包括已有模型测试链接、模型参数存入数据库等。参考原有的microDify模块架构规划，使用对应的数据库表单，如果现有表单不能完成，则额外设计表单，必须遵循claude.md的约束条件。限制性要求：1.必须遵守claude.md的规范 2.先在对话中显示 3.有任何问题使用头脑风暴对我进行提问。

点击测试没有什么反应，从数据链路流动路径，一步一步进行排查问题

查看/provider（模型管理） 相关前端和后端内容还要什么可能需要修改或者优化的地方

使用opus4.8进行代码优化：
分析我的microDify项目前端内容和后端内容，前端内容的样式要求为@docs/design-system.html 和@docs/deployment.md 中。我已经实现了部分的业务代码，包括用户登入、用户管理、模型管理、仪表盘等，分析一下这些业务的代码是否符合claude.md的规范，业务之间的有无冲突，有无可以优化的地方。

在 Provider 模块的数据模型中，多种供应商鉴权方式的差异怎么统一存储、一个供应商下有多个模型怎么管理、供应商健康状态怎么表示。

根据microDify的项目使用的技术栈，可以参考claude.md，生成一段prompt可以实现上面provider模块数据模型的核心设计中的多种鉴权方式的统一存储，使其符合本项目整体结构，先在对话中输出。限制性要求：1.必须遵守claude.md的规范 2.先在对话中显示 3.有疑问可以先使用头脑风暴对我进行提问 

根据microDify的项目使用的技术栈，可以参考claude.md，生成一段prompt可以实现上面provider模块数据模型的核心设计中的一个供应商下多个模型的管理，使其符合本项目整体结构，先在对话中输出。限制性要求：1.必须遵守claude.md的规范 2.先在对话中显示 3.有疑问可以先使用头脑风暴对我进行提问 

根据microDify的项目使用的技术栈，可以参考claude.md，生成一段prompt可以实现上面provider模块数据模型的核心设计中的供应商健康状态表示，使其符合本项目整体结构，先在对话中输出。限制性要求：1.必须遵守claude.md的规范 2.先在对话中显示 3.有疑问可以先使用头脑风暴对我进行提问 
