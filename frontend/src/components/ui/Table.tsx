...
function TableFooter({ total, page, onPageChange }: TableFooterProps) {
  return (
    <div className="flex items-center justify-between px-6 py-4 border-t-2 border-[var(--color-divider)] text-[13px] text-[var(--color-text-secondary)]">
      ...
      <div className="flex gap-1">
        <button
          className="w-9 h-9 border-2 border-[var(--color-border)] rounded-[var(--radius-sm)] bg-[var(--color-surface)] font-semibold text-[13px] cursor-pointer transition-all duration-150 hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]"
          disabled={page <= 1}
