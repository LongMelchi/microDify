...
export function Select({ label, options, value, onChange, required, className = "" }: SelectProps) {
  ...
  return (
    <div className={`mb-5 max-w-[480px] ${className}`}>
      {label && (
      ...
      <div className="relative group">
        ...
        >
          {selected?.label || "请选择"}
          <span className="absolute right-3.5 top-1/2 -mt-[3px] w-2 h-2 border-r-2 border-b-2 border-[var(--color-text-secondary)] rotate-45" />
        </button>
        <ul
          className="absolute top-[calc(100%+4px)] left-0 right-0 z-50 bg-[var(--color-surface)] border-[3px] border-[var(--color-text)] rounded-[var(--radius-md)] shadow-[4px_4px_0_rgba(26,26,46,0.10)] list-none max-h-[200px] overflow-y-auto p-1 hidden group-focus-within:block"
        >
          {options.map((opt) => (
            <li
              key={opt.value}
